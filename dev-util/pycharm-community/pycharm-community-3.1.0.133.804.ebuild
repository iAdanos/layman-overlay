...
DESCRIPTION="PyCharm is an intelligent Python IDE"
...
if [ -z $MY_PRE ]; then
	...
	if [[ "${VER[4]}" == "0" ]]; then
		if [[ "${VER[2]}" == "0" ]]; then
			SRC_URI="http://download.jetbrains.com/python/${PN}-$(get_version_component_range 1-1).tar.gz"
		else
			SRC_URI="http://download.jetbrains.com/python/${PN}-$(get_version_component_range 1-2).tar.gz"
		fi
	else
		SRC_URI="http://download.jetbrains.com/python/${PN}-$(get_version_component_range 1-3).tar.gz"
	fi
else
...
#	ln -s /etc/idea/idea-${SLOT}.properties bin/idea.properties
	...
	make_desktop_entry ${exe} "PyCharm ${PV}" "${exe}" "Development;IDE"
